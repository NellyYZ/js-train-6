...
function printKeysAndValues(obj) {
  for (const key in country) {
    ...
    console.log(key, country[key]); // Виводимо ключ та значення на консоль
  }
}
...
// user.introduce; // Розкоментуйте рядок нижче після виконня завдання для перевірки
...
function destructureObject(obj) {
  let { brand, model } = laptop; // Використовуємо деструктуризацію для створення нових змінних з властивостей об'єкту і отримуємо з нього змінні brand та model
...
function printProductDetails(obj) {
  ...
  } = product; // Використовуємо деструктуризацію для отримання значень productName, price i також значень companyName, country вкладеного об'єкту manufacturer
